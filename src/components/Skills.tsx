...
export const Skills: React.FC = () => {
  ...
  const getIconForSkill = (skillName: string) => {
    ...
    if (lowerName.includes("react") || lowerName.includes("redux"))
    ...
    if (
      ...
      return <Zap className="text-indigo-400" />;
    if (lowerName.includes("tailwind") || lowerName.includes("scss"))
      return <GitBranch className="text-indigo-400" />;
    if (lowerName.includes("graphql") || lowerName.includes("firebase"))
      return <Globe className="text-indigo-400" />;
    return <Code className="text-indigo-400" />;
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
          {skills.map((skill: SkillItem, index: number) => (
            <div
              key={index}
              className="flex flex-col items-start p-5 bg-gray-800 rounded-xl shadow-lg border border-gray-700 hover:bg-gray-700 transition-colors duration-300 group"
            >
              <div className="w-full flex justify-between items-center mb-3">
                <div className="p-2 bg-indigo-900/50 rounded-lg group-hover:bg-indigo-700/50 transition-colors">
                  {React.cloneElement(getIconForSkill(skill.name), {
                    className: "w-8 h-8",
                  })}
                </div>
